...
HungarianBallsRandomSpawner::HungarianBallsRandomSpawner(uint8_t numberOfBalls) : HungarianBallsSpawner(numberOfBalls)
{
}

QPair<HungarianBallsRandomSpawner::BallCollection, HungarianBallsRandomSpawner::BallCollection>HungarianBallsRandomSpawner::spawn()
{
    ...
    auto createBallsAroundHungarianRing = [&](HungarianEllipse* hungarianRing, uint from, uint to, QBrush randomColoredBrush)
    {
        ...
        for (auto radCoeff = from; radCoeff <= to; radCoeff++) {
            ...
            switch (i) {
            case 1:
                randomColoredBrush.setColor(QColor(Qt::red));
                break;
            case 2:
                randomColoredBrush.setColor(QColor(Qt::yellow));
                break;
            case 3:
                randomColoredBrush.setColor(QColor(Qt::blue));
                break;
            }

            newBall->setBrush(randomColoredBrush);
            ballList.push_back(newBall);
        }
    };

    // Рисуем левый внешний набор шаров
    createBallsAroundHungarianRing(&m_leftRing, 4, 14, QBrush(QColor(Qt::red))); // 4-14
